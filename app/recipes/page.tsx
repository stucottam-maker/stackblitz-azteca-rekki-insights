...
function getRecipeStatus(recipe: Recipe) {
  ...
  if (recipe.ingredients.some((item) => item.quantity == null)) {
...
export default function RecipesPage() {
  const prepRecipes = recipes.filter((recipe) => recipe.type === "Prep");
  const menuRecipes = recipes.filter((recipe) => recipe.type === "Menu");
    ...
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav">
          ...
          <Link className="nav-link nav-link-active" href="/recipes">
            <span className="nav-icon">◇</span>
            Recipes
          </Link>
      ...
      <section className="main-content">
        ...
        <section className="stats-grid">
          ...
          <article className="stat-card">
            ...
            <p className="stat-change neutral">Sauces, marinades and batches</p>
          </article>

          <article className="stat-card">
            <p className="stat-label">Menu recipes</p>
            <p className="stat-value">{menuRecipes.length}</p>
            <p className="stat-change neutral">Linked to menu dishes</p>
          </article>

          <article className="stat-card">
            <p className="stat-label">Costed recipes</p>
            <p className="stat-value">0</p>
            <p className="stat-change warning">Ingredient prices needed</p>
          </article>
        </section>
